...
import { User, Mail, Phone, MapPin, Building2, GraduationCap, Save } from "lucide-react";
...
export default function StudentSettingsPage() {
  ...
  const handleSave = async () => {
    await updateProfile({ name, phone, location, college, degree });
  ...
  const fields = [
    ...
    { label: "College", value: college, onChange: setCollege, icon: <Building2 className="w-5 h-5" />, disabled: false },
    { label: "Degree", value: degree, onChange: setDegree, icon: <GraduationCap className="w-5 h-5" />, disabled: false },
  ];
